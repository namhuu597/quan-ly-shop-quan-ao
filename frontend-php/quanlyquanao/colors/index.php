<?php

require_once __DIR__ . '/../includes/auth.php';

requirePermission('color.read');

$pageTitle = 'Quản lý Màu sắc';
$currentPage = 'colors';

$apiUrl =
    'http://localhost:5162/api/colors';

$colors = [];
$apiError = '';

$ch = curl_init();

curl_setopt_array(
    $ch,
    [
        CURLOPT_URL =>
            $apiUrl,

        CURLOPT_RETURNTRANSFER =>
            true,

        CURLOPT_CONNECTTIMEOUT =>
            5,

        CURLOPT_TIMEOUT =>
            10,

        CURLOPT_HTTPHEADER =>
            apiAuthHeaders()
    ]
);

$response =
    curl_exec($ch);

$httpCode =
    (int)curl_getinfo(
        $ch,
        CURLINFO_HTTP_CODE
    );

if ($response === false) {

    $apiError =
        'Không thể kết nối tới ASP.NET Core API.';

} elseif ($httpCode !== 200) {

    $apiError =
        'Không thể tải danh sách màu sắc. HTTP Status: '
        . $httpCode;

} else {

    $data =
        json_decode(
            $response,
            true
        );

    if (is_array($data)) {

        $colors = $data;

    } else {

        $apiError =
            'Dữ liệu API trả về không hợp lệ.';
    }
}

curl_close($ch);


require_once __DIR__ . '/../includes/header.php';
require_once __DIR__ . '/../includes/sidebar.php';

?>

<main class="main">

    <?php
    require_once __DIR__ . '/../includes/topbar.php';
    ?>

    <section class="content">


        <?php if ($apiError !== ''): ?>

            <div class="alert alert-danger">
                <?= htmlspecialchars($apiError) ?>
            </div>

        <?php endif; ?>


        <?php if (
            isset($_GET['created'])
            && $_GET['created'] === '1'
        ): ?>

            <div class="alert alert-success">
                Thêm màu sắc thành công!
            </div>

        <?php endif; ?>


        <?php if (
            isset($_GET['updated'])
            && $_GET['updated'] === '1'
        ): ?>

            <div class="alert alert-success">
                Cập nhật màu sắc thành công!
            </div>

        <?php endif; ?>


        <?php if (
            isset($_GET['status_changed'])
            && $_GET['status_changed'] === '1'
        ): ?>

            <div class="alert alert-success">
                Cập nhật trạng thái màu sắc thành công!
            </div>

        <?php endif; ?>


        <div class="product-page-header">

            <div>

                <h1>Danh sách Màu sắc</h1>

                <p>
                    Màu sắc / Danh sách
                </p>

            </div>


            <?php if (
                hasPermission('color.create')
            ): ?>

                <a
                    href="/quanlyquanao/colors/create.php"
                    class="btn btn-primary"
                >
                    + Thêm màu
                </a>

            <?php endif; ?>

        </div>


        <div class="card">

            <div class="table-responsive">

                <table class="table">

                    <thead>

                        <tr>
                            <th>#</th>
                            <th>Màu</th>
                            <th>Tên màu</th>
                            <th>Mã màu</th>
                            <th>Số biến thể đang dùng</th>
                            <th>Trạng thái</th>
                            <th>Thao tác</th>
                        </tr>

                    </thead>


                    <tbody>

                    <?php if (
                        $apiError === ''
                        && empty($colors)
                    ): ?>

                        <tr>
                            <td
                                colspan="7"
                                class="empty-data"
                            >
                                Chưa có màu sắc nào.
                            </td>
                        </tr>

                    <?php elseif (
                        $apiError === ''
                    ): ?>

                        <?php foreach (
                            $colors as $index => $color
                        ): ?>

                            <tr>

                                <td>
                                    <?= $index + 1 ?>
                                </td>


                                <td>

                                    <span
                                        class="color-preview"
                                        style="background:
                                            <?= htmlspecialchars(
                                                !empty($color['colorCode'])
                                                    ? $color['colorCode']
                                                    : '#cccccc'
                                            ) ?>;"
                                    ></span>

                                </td>


                                <td>

                                    <strong>
                                        <?= htmlspecialchars(
                                            $color['name']
                                            ?? ''
                                        ) ?>
                                    </strong>

                                </td>


                                <td>

                                    <?= htmlspecialchars(
                                        !empty($color['colorCode'])
                                            ? $color['colorCode']
                                            : '—'
                                    ) ?>

                                </td>


                                <td>

                                    <?= (int)(
                                        $color['variantCount']
                                        ?? 0
                                    ) ?>

                                </td>


                                <td>

                                    <?php if (
                                        !empty(
                                            $color['isActive']
                                        )
                                    ): ?>

                                        <span class="status status-success">
                                            Đang sử dụng
                                        </span>

                                    <?php else: ?>

                                        <span class="status status-danger">
                                            Ngừng sử dụng
                                        </span>

                                    <?php endif; ?>

                                </td>


                                <td>

                                    <div class="table-actions">

                                        <?php if (
                                            hasPermission(
                                                'color.update'
                                            )
                                        ): ?>

                                            <a
                                                href="/quanlyquanao/colors/edit.php?id=<?= (int)($color['id'] ?? 0) ?>"
                                                class="action-btn"
                                                title="Sửa"
                                            >
                                                ✏️
                                            </a>

                                        <?php endif; ?>


                                        <?php if (
                                            hasPermission(
                                                'color.delete'
                                            )
                                        ): ?>

                                            <form
                                                method="POST"
                                                action="/quanlyquanao/colors/toggle-status.php"
                                                onsubmit="return confirm(
                                                    'Bạn có chắc muốn thay đổi trạng thái màu này?'
                                                );"
                                            >

                                                <?= csrfField() ?>

                                                <input
                                                    type="hidden"
                                                    name="id"
                                                    value="<?= (int)($color['id'] ?? 0) ?>"
                                                >

                                                <?php if (
                                                    !empty(
                                                        $color['isActive']
                                                    )
                                                ): ?>

                                                    <button
                                                        type="submit"
                                                        class="action-btn action-danger"
                                                    >
                                                        ⛔
                                                    </button>

                                                <?php else: ?>

                                                    <button
                                                        type="submit"
                                                        class="action-btn action-success"
                                                    >
                                                        ✅
                                                    </button>

                                                <?php endif; ?>

                                            </form>

                                        <?php endif; ?>

                                    </div>

                                </td>

                            </tr>

                        <?php endforeach; ?>

                    <?php endif; ?>

                    </tbody>

                </table>

            </div>

        </div>

    </section>

</main>

<?php
require_once __DIR__ . '/../includes/footer.php';
?>